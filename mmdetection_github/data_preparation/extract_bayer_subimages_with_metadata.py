...
from process import raw2rgb_v2
...
def main():
    ...
    parser.add_argument('--iso', action='store_true', help='if you want to use iso metadata.')
    ...
    opt['iso'] = True if args.iso else False
...
def worker(path, opt):
    ...
    np.savez(osp.join(opt['save_folder'], f'{img_name}'),
                im=img,
