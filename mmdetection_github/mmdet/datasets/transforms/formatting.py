...
class PackDetInputs(BaseTransform):
    ...
    def __init__(self,
                 meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape',
                            'scale_factor', 'flip', 'flip_direction'),
                 bayer_substitute = None,
                 suffix = '.png'):
        self.meta_keys = meta_keys
        self.bayer_substitute = bayer_substitute
        self.suffix=suffix
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'img' in results:
            ...
            if not img.flags.c_contiguous:
                ...
                if 'gt' in results.keys():
                    ...
                    gt = to_tensor(gt)
                    packed_results['gts'] = gt
            else:
                img = to_tensor(img).permute(2, 0, 1).contiguous()
                if 'gt' in results.keys():
                    gt = results['gt']
                    gt = to_tensor(gt).permute(2, 0, 1).contiguous()
                    packed_results['gts'] = gt
        ...
        img_meta = {}
        if self.bayer_substitute is not None:
            file_name = os.path.basename(results['img_path']).replace('.npz', self.suffix)
            # match = re.search(r'(\d+)', file_name)
            # if match:
            #     number = int(match.group(1)) - 1
            #     file_name = file_name.replace(match.group(1), str(number))
            full_path = os.path.join(self.bayer_substitute, file_name)
            results['img_path'] = full_path
