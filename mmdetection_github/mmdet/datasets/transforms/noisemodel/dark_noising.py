...
class NoiseModel:
    def __init__(self, param_dir=None, model='g', camera='CanonEOS5D4', cfa='bayer', 
                ...
                dark_ratio=(0, 1.0), noise_ratio=(10, 100)):
        super().__init__()
        modes = ['addnoise', 'unprocess_addnoise', 'unprocess', 'only_noise']
        ...
        if param_dir is None:
            try:
                self.param_dir = '/home1/guojs/codes/Dark-ISP/mmdetection_github/mmdet/datasets/transforms/noisemodel/camera_params'
            except:
    ...
    def unprocess(self, image, addnoise=True, return_clean=False):
        ...
        raw, ccm, wb = unprocess(image)  # unprocess成四通道raw，按RGBG排列, -> 0~1
        # Image.fromarray(raw).show()
        # print(raw.shape)

        if addnoise :
            # self.add_noise 会归一化
            noisy_raw, saturation_level, ratio = self.add_noise(raw.copy())  # 添加噪声
            ...
            return dark_raw, ccm, wb, saturation_level, ratio
        else:
...
class AddNoisyImg(NoiseModel):
    def __init__(self, param_dir=None, model='g', camera='CanonEOS5D4', cfa='bayer', 
                ...
                dark_ratio=(0, 1.0), noise_ratio=(10, 100), only_noise=True):
    ...
    def __call__(self, results, params=None):
        ...
        if self.mode == 'addnoise':
            if self.use_255:
                results['noisy_img'] = self.add_noise_255(img, params) * _dark
            else:
                results['noisy_img'] = self.add_noise(img, params) * _dark
        elif self.mode == 'unprocess':
            results['img'], ccm, wb, sl, ratio = self.unprocess(img, addnoise=False)
            results['img'] = results['img'] * _dark
            
            black_level = torch.from_numpy(np.array([0, 0, 0, 0], dtype=float)).float().contiguous()
            white_level = torch.from_numpy(np.array([sl, sl, sl, sl], dtype=float)).float().contiguous()
            wb = torch.from_numpy(wb).float().contiguous()
            ccm = torch.from_numpy(ccm).float().contiguous()
            iso = torch.tensor(np.array([200], dtype=float), dtype=torch.float)
            exp_time = torch.tensor(np.array([0.04], dtype=float), dtype=torch.float)
            ...
            noisy_img, ccm, wb, sl, ratio = self.unprocess(img, addnoise=True, return_clean=True)
            results['noisy_img'] = noisy_img * _dark
            results['img'] = results['img'] * _dark
