    ...
    def forward(self, data: dict, training: bool = False) -> Union[dict, list]:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            for i in range(len(_batch_inputs)):
                ...
                if 'gt' in data['data_samples'][i].metainfo.keys():
                    _batch_gt = data['data_samples'][i].metainfo['gt']
                    batch_gts.append(_batch_gt)
                batch_inputs.append(_batch_input)
            # Pad and stack Tensor.
            batch_inputs = stack_batch(batch_inputs, self.pad_size_divisor,
                                       self.pad_value, skip_pad=self.skip_pad)
            if 'gt' in data['data_samples'][i].metainfo.keys():
                device = batch_inputs.device
                batch_gts = stack_batch(batch_gts, self.pad_size_divisor,
                                       self.pad_value)
                data['gts'] = batch_gts.to(device)
