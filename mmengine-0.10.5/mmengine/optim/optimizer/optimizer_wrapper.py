...
class OptimWrapper(BaseOptimWrapper):
    ...
    def update_twoloss(self,
                       detect_loss: torch.Tensor,
        ...
        if zero_kwargs is None:
            zero_kwargs = {}
        detect_loss = self.scale_loss(detect_loss)
        denoise_loss = self.scale_loss(denoise_loss)
        self.backward_twoloss(detect_loss, denoise_loss,
                              partial_models)
        # Update parameters only if `self._inner_count` is divisible by
        # `self._accumulative_counts` or `self._inner_count` equals to
        # `self._max_counts`
    ...
    def backward_twoloss(self, detect_loss: torch.Tensor, denoise_loss: torch.Tensor, 
                         partial_models, lambda_ = 1,  **kwargs):
        # 清零梯度
        ...
        denoise_grads = {}
        for module in partial_models:
            for n, p in module.named_parameters():
                denoise_grads[n] = p.grad.clone()
                p.grad.zero_()
        
        detect_loss.backward()
        for module in partial_models:
            for n, p in module.named_parameters():
                denoise_grad = denoise_grads[n]
                denoise_grad_norm = denoise_grad / torch.linalg.norm(denoise_grad)
                a_grad = p.grad.clone()
                a_grad_norm = a_grad / torch.linalg.norm(a_grad)

                if torch.dot(a_grad_norm.flatten(), denoise_grad_norm.flatten()) < 0:
                    p.grad = a_grad - lambda_ * torch.dot(
                        a_grad.flatten(), denoise_grad_norm.flatten()
                    ) * denoise_grad_norm
                else:
                    p.grad = torch.dot(a_grad.flatten(), denoise_grad_norm.flatten()) * denoise_grad_norm
